...
/// Window implementation [glfw backend].

class window::impl {
public:
    impl(GLFWwindow* glfw_window) 
...
static auto _glfw_char_callback(GLFWwindow* glfw_window, unsigned int codepoint) -> void {
    //TORNASOL_ASSERT(glfwGetWindowUserPointer(glfw_window) != nullptr);
    

    auto* sol_window = static_cast<sol::window*>(glfwGetWindowUserPointer(glfw_window));
    
    if (sol_window) {
        std::printf("Ok, window found");
    }
    else {
        std::printf("Nope");
    }

    if (sol_window->on_input_char) {
        sol_window->on_input_char(codepoint);
    }
}

/// Window pointer to implementation [pimpl idiom].
...
window::window(window&& other) noexcept
    : on_input_char(std::move(other.on_input_char)), 
...
auto make_window(std::string_view title, isize2 size) -> result<window> {
    ...
    if (!glfw_window) {
        return std::unexpected { error{ "Failed to create window `glfw.create_window`" } };
    }

    // Construct the sol::window.
    auto sol_window = window { std::make_unique<window::impl>(glfw_window) };

    // Setup the callbacks for the window.    
    glfwSetWindowUserPointer(glfw_window, &sol_window);
    glfwSetCharCallback(glfw_window, _glfw_char_callback);
